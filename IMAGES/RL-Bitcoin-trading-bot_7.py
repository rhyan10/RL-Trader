...
class CustomEnv:
    ...
    def step(self, action):
        ...
        if action == 0:  # Hold
            ...
            self.trades.append({'High': High, 'Low': Low, 'total': self.crypto_bought, 'type': "buy",
            ...
            self.trades.append({'High': High, 'Low': Low, 'total': self.crypto_sold, 'type': "sell",
                                'current_price': current_price})
            self.episode_orders += 1
...
def Random_games(env, visualize, test_episodes=50, comment=""):
    ...
    for episode in range(test_episodes):
        ...
        while True:
            ...
            if env.current_step == env.end_step:
                ...
                print("episode: {}, net_worth: {}, average_net_worth: {}, orders: {}".format(episode, env.net_worth,
                                                                                             average_net_worth / (
                                                                                                         episode + 1),
                                                                                             env.episode_orders))
                break
    ...
    print(
        "average {} episodes random net_worth: {}, orders: {}".format(test_episodes, average_net_worth / test_episodes,
...
def test_agent(test_df, test_df_nomalized, visualize=True, test_episodes=10, folder="", name="", comment="",
...
if __name__ == "__main__":
    ...
    df2 = pd.read_csv('BTC15m.csv')
    df = df.drop(df.columns[[0]], axis=1)
    df = df.dropna()
    df = df.sort_values('Date')
    df = AddIndicators(df, df2)  # insert indicators to df 2021_02_21_17_54_Crypto_trader
    ...
    train_df_nomalized = df_nomalized[
                         :-test_window - lookback_window_size]  # we leave 100 to have properly calculated indicators
    ...
    agent = CustomAgent(lookback_window_size=lookback_window_size, lr=0.00001, epochs=5, optimizer=Adam, batch_size=32,
                        model="CNN", depth=depth, comment="Normalized")
    train_multiprocessing(CustomEnv, agent, train_df, train_df_nomalized, num_worker=30, training_batch_size=500,
                          visualize=False, EPISODES=400000)

    # test_multiprocessing(CustomEnv, CustomAgent, test_df, test_df_nomalized, num_worker = 1, visualize=False, test_episodes=1000, folder="2021_06_25_23_42_Crypto_trader", name="1607.14_Crypto_trader", comment="3 months")
